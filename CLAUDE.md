...
- **Kredi modeli (API reselling — subscription + quota):** SelfAI Affogato API'sini wrap eder ve kendi paketler hâlinde satar. Tek bir Affogato havuzu (örn. Ultra 7.000 credit/ay) tüm kullanıcılar tarafından paylaşılır; SelfAI içinde her kullanıcının kendi TokenWallet'ı vardır. Paketler: **Free=5 credit (one-time bonus, yenilenmez)**, Basic/Plus/Premium (aylık subscription, yenilenir). Her generation kullanıcının cüzdanından düşülür, Affogato çağrısı sonucuna göre commit veya refund. Aşırı satım riski sen yönetirsin (toplam aktif subscription kredisi ≤ Affogato kapasitesi). **Identity sistemi: Firebase Auth, ASP.NET Identity YOK.** AppUser kendi domain entity'mizdir, FirebaseUid ile bağlanır.
- **Iyzico configuration section:** Iyzico key'leri User Secrets'te `"IyzicoOptions"` section'ı altında saklanır (önceki User Secrets migration'dan miras), `"Iyzico"` DEĞİL. Service tarafında `IyzicoOptions` POCO class'ı `IOptions<IyzicoOptions>` ile inject edilir. Yeni bir `IyzicoSettings` class'ı YARATILMAYACAK.
...
- **Frontend MVC separation (kesin kural):** Razor view'lar (.cshtml) yalnızca markup içerir. JavaScript logic'i `wwwroot/js/{feature}.js` altında ayrı dosyalarda yaşar (IIFE module pattern, mevcut konvansiyon). CSS kuralları `wwwroot/css/` altında veya `tailwind.css` source'unda `@layer components` içinde tutulur. View'larda `<script>` bloğu içinde logic YASAK; `<style>` bloğu YASAK; inline event handler (onclick=, onchange= vb.) YASAK. `@section Scripts` yalnızca harici dosyayı referans almak için kullanılır (`<script src="~/js/...js"></script>`). Yeni frontend görevlerinde bu kurala kesinlikle uyulur, mevcut inline script bulunan view'lar dokunulduğunda fırsat görüldükçe refactor edilir.
- **Tasarım dili: Liquid Glass (tüm uygulama):** Frontend redesign sprint'inde tüm SelfAI UI "liquid glass" tasarım diline geçer — yarı şeffaf cam yüzeyler (rgba alpha + backdrop-filter blur), refraktif kenarlar (1px subtle highlight borders), soft glow efektleri, layered depth, gradient overlay'lar. Mevcut solid/flat #1E1E1E surfaces transitionlanacak. **Studio (F.4) showcase olarak başlar ama Login, Pricing, History, tüm sayfalar aynı dile uyar** — tutarlı görsel hierarchy. Reference: Apple Liquid Glass (WWDC 2025), modern glassmorphism. Dark background varsayılan, accent rengi #00CED1 cam üzerinden parlar. Hover/focus state'leri "alive" hissi vermeli (smooth transitions, subtle scale/glow).
- **Kilitli design token kararları (design-tokens.md'nin PART 2'sinin authoritative cevapları):** (1) Tek accent rengi `#00CED1` — indigo `#6366f1/#8b5cf6` tüm kullanım noktalarından kaldırılır. (2) Tek page background `#0A0A0A` — `_Layout` body'deki `#1a1a1a`, Pricing wrapper'ları, hepsi bu değere migrate. (3) Page background'a subtle radial gradient: `radial-gradient(ellipse at top, rgba(0, 206, 209, 0.05), #0A0A0A 60%)` — accent aurora hissi. (4) Glass surface alpha: beyaz/nötr alpha (rgba(255,255,255,X)); turkuaz tint yalnızca aktif/seçili öğelerde. (5) Border solid gerektiğinde tek değer `rgba(255, 255, 255, 0.12)`. (6) Tailwind strategy: paralel `glass-*` class'ları eklenir, eski class'lar her görevde fırsat buldukça migrate edilir (aniden remap YOK). (7) Inter font weight 700 import edilir. (8) Focus state: `:focus-visible` ile 2px ring `rgba(0, 206, 209, 0.5)` + glow birlikte (sadece glow YETERSİZ, klavye erişilebilirliği şart).
- **Affogato benzeri sayfa yapısı (gelecek frontend görevleri için target):** SelfAI Affogato'nun (affogato.ai → app.affogato.ai/login → app.affogato.ai) sayfa yapısını model alır. (a) `/` route: **Landing page** — marketing/showcase, CTA butonu (şu an `/` RenderNet/Index'e gidiyor, F.0'da değişir). (b) `/Account/Login` (veya `/login`): **Dedicated full-page login** — sol hero + sağ form split layout (mevcut Home/Index modal yerine, F.2'de yapılır, Bootstrap dependency de kalkar). (c) `/Studio` (mevcut RenderNet/Index): **Ana uygulama** — Login sonrası landing. Mevcut route adı korunabilir veya rename edilebilir (rename ileride değerlendirilir). Sidebar nav: Studio / Canvas / Characters / Pricing.
