...
namespace SelfAI.Services.Generation.Providers.Legacy.Affogato
{
    public class RenderNetGenerationService : IRenderNetGenerationService
    {
        ...
        public async Task<ServiceResult<GenerateMediaResponseDto>> GenerateMediaAsync(MediaGenerationRequestDto dto)
        {
            ...
            // style_detail.base_model: API'nin stili doğru çözümlemesi için zorunlu.
            // Düz "style" string'i base_model bağlamı olmadan gönderilince API stili
            // default base_model altında arayıp bulamaz → GE09 "Invalid styles".
            // Tüm style kartları şu an aynı base_model (flux) altında; tek değer hepsine uygulanır.
            // Frontend StyleDetail.BaseModel hidden input'unu seçili karttan doldurur.
            var styleBaseModel = string.IsNullOrWhiteSpace(dto.StyleDetail?.BaseModel)
                ? "flux"
                : dto.StyleDetail.BaseModel.Trim();
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                payloadElements.Add(BuildGenerationElement(dto, modelForThis, styleForThis, styleBaseModel));
            }
        ...
        /// API kuralı gereği segment/style/model/... mutually exclusive — yalnızca biri gönderilir:
        ///  - style doluysa: style_detail { name, base_model } koyulur, model/düz style koyulmaz.
        ///    (Düz "style" string'i base_model bağlamı taşımadığı için GE09 "Invalid styles" verir;
        ///     style_detail object'i base_model ile stili kesin çözer.)
        ...
        private static object BuildGenerationElement(
            MediaGenerationRequestDto dto, string? model, string? style, string baseModel)
        {
            var promptObj = new
            {
